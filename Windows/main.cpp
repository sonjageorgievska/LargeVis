...
long long if_embed = 1, out_dim = -1, n_samples = -1, n_threads = -1, n_negative = -1, n_neighbors = -1, n_trees = -1, n_propagation = -1, to_log = -1;//to_log added by Sonja
...
int main(int argc, char **argv)
{
	...
	if (argc < 3)
	{
		...
		printf("-log: specify whether the input similarities should be pre-processed with logarithmization (1) or not (0). Default is 0.\n");//added by Sonja
		return 0;
	}
	if ((i = ArgPos((char *)"-fea", argc, argv)) > 0) if_embed = atoi(argv[i + 1]);
	if ((i = ArgPos((char *)"-input", argc, argv)) > 0) strcpy_s(infile, argv[i + 1]);
	if ((i = ArgPos((char *)"-output", argc, argv)) > 0) strcpy_s(outfile, argv[i + 1]);
	...
	if ((i = ArgPos((char *)"-log", argc, argv)) > 0) to_log = atoi(argv[i + 1]);//added by Sonja
	...
	model.run(out_dim, n_threads, n_samples, n_propagation, alpha, n_trees, n_negative, n_neighbors, n_gamma, perplexity, to_log);//to_log added by Sonja
